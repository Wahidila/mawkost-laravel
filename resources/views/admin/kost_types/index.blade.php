...
<div class="bg-white/90 backdrop-blur-xl w-full shadow-[0_4px_12px_rgba(92,61,46,0.08)] rounded-2xl border border-primary-lighter/30 overflow-hidden">
    <div class="p-6 border-b border-primary-lighter/40 bg-white/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-lg font-bold font-display text-primary-dark tracking-tight">Daftar Tipe Kost</h3>
            <p class="text-sm text-gray-500 mt-0.5">Atur klasifikasi tipe kost yang tersedia.</p>
        </div>
        <form action="{{ route('admin.kost_types.store') }}" method="POST" class="flex gap-2 w-full sm:w-auto">
            @csrf
            <input type="text" name="name" placeholder="Nama Tipe Baru..." class="border border-primary-lighter rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-primary-lighter/10 flex-1 min-w-[180px] transition-all placeholder:text-gray-400" required>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-primary-lighter/50">
            <thead class="bg-primary-lighter/20">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-bold text-primary-dark uppercase tracking-wider font-display">Nama Tipe</th>
                ...
                <tr class="hover:bg-primary-lighter/10 transition-colors duration-150">
                    ...
                    <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('admin.kost_types.edit', $item->id) }}" class="text-primary hover:text-cta transition-colors flex items-center gap-1.5">
                ...
                <tr>
                    <td colspan="4" class="px-6 py-8 whitespace-nowrap text-sm text-center font-medium bg-gray-50/30">
                        <div class="flex flex-col items-center justify-center">
                            <i class="fa-solid fa-layer-group text-3xl mb-3 text-primary-light"></i>
                            <p class="text-gray-500">Belum ada tipe kost.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
